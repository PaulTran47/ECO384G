...
data3 = xlsread(append(data_dir, '\FEDFUNDS.xls'));
%==========================================================================
...
% Making FFR and MP shock follow sample period of 1970m1 - 1996m12.
...
% Doing first step of 2SLS regression.
regressors_1s = [ones(size(ffr)) chg_ln_ip_sal mp_shock];
b_1s = regress(ffr, regressors_1s);
ffr_hat = regressors_1s*b_1s;

% Doing second step of 2SLS regression, which gives individual IRs for each
% horizon.
for i = 1:h
  % Creating variables for 2SLS regression.
  ...
  regressors = [chg_ln_ip_sal ffr_hat];
  
  % Doing second step of 2SLS regression. Obtaining coefficients and 95% CI
